...
if [ -z "$concat_modules" ] ; then
    interesting_modules=( $(cd $lambdapidir/src/core ; ls --hide=dune | cut -d .  -f-1) )
#  timed compile handle infer proof sr tactics typing unif inductive queries roro)
			  #basics scope terms builtin console ctxt env eval external files hrs package pretty print queries sign sig_state)
else
  interesting_modules=( $@ )
fi
