...
colorscheme=( ${paired12[@]} ${piyg11[@]} ${brbg11[@]} )
...
for mldependency in ${interesting_modules[@]} ; do
  ...
  for mlfile in ${interesting_modules[@]} ; do
    ...
    if [ $mlfile != $mldependency ] ; then
      ...
      if [ ! -z "$mlfiledependsonmldependency" ] ; then
        cd $lambdapidir/_build/default
...
# GENERATE THE GRAPH
cd $workingdir
cat $graphdot | dot -Tpng -o $graphpng
