...
let status : sym -> symb_status = fun s ->
  ...
  let rec is_arrow_kind : Core.Terms.term -> bool -> symb_status = fun t b ->
    match t with
...
let print_rule : Format.formatter -> int -> sym -> rule -> unit =
  ...
  let lhs = Core.Basics.add_args (Symb s) r.lhs in
  Format.fprintf oc "<rule>@.<lhs>@.%a</lhs>@." (print_term i s.sym_name) lhs;
  let rhs = Core.Basics.term_of_rhs r in
...
let get_vars : sym -> rule -> (string * Core.Terms.term) list = fun s r ->
