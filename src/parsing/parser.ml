...
let parser_fatal : pos -> ('a,'b) File_management.Error.koutfmt -> 'a = fun loc fmt ->
  File_management.Error.fatal (Some(loc)) fmt
    ...
    fun ?inchan fname lexbuf ->
      ...
      let generator _ =
        ...
        | LpParser.Error ->
            ...
            let loc = File_management.Pos.locate loc in
...
let parse_qident : string ->
  ...
  let parse_ident (s: string): (ident * bool, popt) result =
    ...
    with LpLexer.SyntaxError(s) -> Error(s.pos)
       | LpParser.Error ->
         let loc = File_management.Pos.locate (Sedlexing.lexing_positions lexbuf) in
         Error(Some(loc))
  in
