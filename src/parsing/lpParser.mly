  ...
  | u=term IN x=term t=preceded(IN, term)?
    {
      let ident_of_term {elt; _} =
        match elt with
          | P_Iden({elt=([], x); pos}, _) -> File_management.Pos.make pos x
          | _ -> $syntaxerror
      in
      match t with
...
command:
  ...
  | REQUIRE p=QID AS a=ident SEMICOLON
      {
        let alias = File_management.Pos.make (fst a).pos ((fst a).elt, snd a) in
...
term:
  ...
  | BACKQUOTE q=term_ident b=binder
      {
        let bder = File_management.Pos.make b.pos (P_Abst(fst b.elt, snd b.elt)) in
